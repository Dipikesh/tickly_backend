  ...
  try {
			const googleRedirectUrl =
				process.env.NODE_ENV === 'production'
					? `https://analytics-api.markopolo.ai/google/auth/callback`
					: `http://localhost:${process.env.DEV_PORT}/oauth/google/validate`;
			const options = {
...
exports.validateSignupCallback = async (req,res,next) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: true, message: "Signed In successfully",data:userInfo });
    }
    catch (error) {
        console.log(error);
        return res.status(500).json({ message: 'Something went wrong' });
